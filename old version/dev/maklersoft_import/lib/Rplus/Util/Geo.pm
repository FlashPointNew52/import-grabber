...
# Получение района
sub get_district_by_coords {
    my ($latitude, $longitude) = @_;
    my ($district, $subdistrict, $metro);
    my $res = $ua->get(
        'https://geocode-maps.yandex.ru/1.x/?geocode=' . $longitude . ', ' . $latitude . '&format=json',
        {}
    )->res->json;
    if ($res && $res->{response}->{GeoObjectCollection}->{metaDataProperty}->{GeocoderResponseMetaData}->{found} > 0) {
        my @featureMember = @{$res->{response}->{GeoObjectCollection}->{featureMember}};
        my @districts;
        foreach(@featureMember){
            if($_->{GeoObject}->{metaDataProperty}->{GeocoderMetaData}->{kind} eq "district"){
                push @districts, $_->{GeoObject}->{name};
            }
            if($_->{GeoObject}->{metaDataProperty}->{GeocoderMetaData}->{kind} eq "metro"){
                $metro = $_->{GeoObject}->{name};
            }
        }

        if(scalar @districts > 1){
            $district = $districts[1];
            $subdistrict = $districts[0];
        } else{
            $district = $districts[0];
        }
    }
    return (district => $district, subdistrict => $subdistrict, metro => $metro);
}
