package Rplus::Import::Queue::Yandex;
...
sub _get_url_list {
    ...
    for(my $i = 1; $i <= $page_count; $i ++) {
        ...
        if ($res && $res->dom) {
            ...
            $dom->find('div[class*="OffersSerpItem__info-inner"]')->each (sub {
                my $item_url = $_->find('div[class*="OffersSerpItem__generalInfo"]')->first->at('a[class*="OffersSerpItem__link"]')->{href};
                my $item_id = basename($item_url);

                my $date_str = $_->find('div[class*="OffersSerpItem__publish-date"]')->first->all_text;
