...
sub _get_url_list {
    ...
    for(my $i = 1; $i <= $page_count; $i ++) {
        ...
        if ($res && $res->dom) {
            ...
            $dom->find('div[class="notice-item-wrapper"] div[class="media-body"]')->each (sub {
                my $item_url = $_->find('div[class="items-bar__group"]')->first->at('a')->{href};
                my $item_id = basename($item_url);
                my $date_str = $_->find('div[class~="text-muted"]')->first->all_text;
