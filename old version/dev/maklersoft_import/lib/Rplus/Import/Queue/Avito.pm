package Rplus::Import::Queue::Avito;
    ...
    for(my $i = 1; $i <= $page_count; $i ++) {
        ...
        if ($res && $res->dom) {
            ...
            $dom->find('div[class~="catalog-list"] div[class~="item"]')->each (sub {
                ...
                my $date_str = $do->at('div[class~="js-item-date c-2"]');
                my $dt_now = DateTime->now();
                my $dt_str = $date_str->{'data-absolute-date'};
                # say '----------------';
                # say 'date: ', $dt_str;
                # $dt_str =~ s/^\s+//;
                # say 'date2: ', $dt_str;
                # my $ts = _parse_date($dt_str);
                # say 'ts: ', $ts;
                #
                # say 'dt_now: ', $dt_now;
                # say '----------------';
                push(@url_list, {id => $item_id, url => $item_url, ts => $dt_now});
