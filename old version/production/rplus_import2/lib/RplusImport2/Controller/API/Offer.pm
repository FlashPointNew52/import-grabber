...
my %source_media = (
        "авито" => "avito",
        "из рук в руки" => "irr",
        "презент архив" => "present_archive",
        "презент сайт" => "present",
        "фарпост" => "farpost",
        "циан"  => "cian"
);
...
sub search {
    ...
    elsif($agent =~ /realtor (.{1,})/ || $agent =~ /realtor/){
        my $content_phones = '';
        my $phones_str = $1;
    ...
    } elsif($agent =~ /phones (.{1,})/ || $agent =~ /phones/){
        my $content_phones = '';
        my $phones_str = '';
        if($1){ $phones_str = $1;}
    ...
    } elsif($sort =~/{\"changeDate\":\"(.+)\"}/ ){
        push(
            @{$full_query{body}{sort}},
            {
                change_date => { order => lc($1), missing => "_last", "unmapped_type" => "long" },
            }
        );
    } elsif($sort =~/{\"ownerPrice\":\"(.+)\"}/ ) {
        push(
            @{$full_query{body}{sort}},
            {
                price => { order => lc($1), "unmapped_type" => "long", missing => "_last"   }
            }
        );
    }
